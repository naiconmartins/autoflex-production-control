...
public class ProductResourceIT {
  ...
  void findById_shouldReturnProduct_whenIdExists() {
    ProductResponseDTO created =
        insertProduct(ProductFactory.createUniqueProductRequestDTOWithRawMaterials());
  ...
  void insert_shouldReturn409_whenCodeIsDuplicate() {
    ...
    ProductRequestDTO request =
        ProductFactory.createProductRequestDTOWithRawMaterials(created.getCode());
  ...
  void delete_shouldReturn204_whenIdExists() {
    ...
    given().pathParam("id", created.getId()).when().delete("/products/{id}").then().statusCode(204);

    given().pathParam("id", created.getId()).when().get("/products/{id}").then().statusCode(404);
  }
  ...
  void update_shouldReturn422_whenDataIsInvalid() {
    ProductResponseDTO created =
        insertProduct(ProductFactory.createUniqueProductRequestDTOWithRawMaterials());
    ProductRequestDTO invalidRequest =
        new ProductRequestDTO("ERR-02", "", new BigDecimal("-1.0"), List.of());
  ...
  void insert_shouldReturn404_whenRawMaterialNotFound() {
    ProductRawMaterialRequestDTO nonexistentMaterial =
        new ProductRawMaterialRequestDTO(999L, new BigDecimal("10.0"));
    ProductRequestDTO request =
        new ProductRequestDTO(
            "PROD-NEW-UNIT", "New", new BigDecimal("10.0"), List.of(nonexistentMaterial));
  ...
  void delete_shouldReturn403_whenUserIsNotAdmin() {
    given().pathParam("id", 1L).when().delete("/products/{id}").then().statusCode(403);
  }
