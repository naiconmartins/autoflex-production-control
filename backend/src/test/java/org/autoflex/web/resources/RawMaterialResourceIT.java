...
public class RawMaterialResourceIT {
  ...
  void insert_shouldReturn201_whenDataIsValid() {
    RawMaterialRequestDTO request =
        new RawMaterialRequestDTO(INITIAL_CODE, "Initial Material", new BigDecimal("100.0"));

    RawMaterialResponseDTO response =
        given()
            ...
            .extract()
            .as(RawMaterialResponseDTO.class);
  ...
  void insert_shouldReturn409_whenCodeAlreadyExists() {
    RawMaterialRequestDTO request =
        new RawMaterialRequestDTO(INITIAL_CODE, "Another Name", new BigDecimal("50.0"));
  ...
  void update_shouldReturn200_whenDataIsValid() {
    RawMaterialRequestDTO updateRequest =
        new RawMaterialRequestDTO(INITIAL_CODE, "Updated Material", new BigDecimal("150.0"));
  ...
  void update_shouldReturn200_whenDataIsValidForAdminRole() {
    ...
    RawMaterialRequestDTO updateRequest =
        new RawMaterialRequestDTO(
            TestData.unique("RM-UPD-ADM"), "Updated By Admin", new BigDecimal("200.0"));
  ...
  void insert_shouldReturn422_whenNameIsBlank() {
    RawMaterialRequestDTO invalidRequest =
        new RawMaterialRequestDTO("CODE", "", new BigDecimal("-10.0"));
  ...
  void findById_shouldReturn404_whenIdDoesNotExist() {
    given().pathParam("id", 9999).when().get("/raw-materials/{id}").then().statusCode(404);
  }
  ...
  void delete_shouldReturn204_whenNoConstraintsExist() {
    ...
    given().pathParam("id", rmId).when().delete("/raw-materials/{id}").then().statusCode(204);

    given().pathParam("id", rmId).when().get("/raw-materials/{id}").then().statusCode(404);
  }
  ...
  void delete_shouldReturn400_whenConstraintViolationOccurs() {
    ...
    given()
        ...
        .body(
            "error",
            containsString("Cannot delete raw material because it is referenced by other records"));
  }
  ...
  void update_shouldReturn409_whenCodeConflicts() {
    ...
    RawMaterialRequestDTO conflictRequest =
        new RawMaterialRequestDTO(codeA, "New Name", BigDecimal.ONE);
  ...
  void update_shouldReturn422_whenStockIsNegative() {
    Number idGen =
        given()
            ...
            .body(
                new RawMaterialRequestDTO(
                    TestData.unique("RM-NEG"), "Negative Stock", BigDecimal.TEN))
            .post("/raw-materials")
            .then()
            .statusCode(201)
            .extract()
    ...
    given()
        ...
        .body(
            new RawMaterialRequestDTO(
                TestData.unique("RM-NEG-UPD"), "Updated", new BigDecimal("-1.0")))
        .pathParam("id", rmId)
        .when()
        .put("/raw-materials/{id}")
        .then()
