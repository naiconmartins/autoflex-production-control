...
public class RawMaterialResourceTest {
  ...
  void delete_shouldReturn401_whenUserIsNotAuthenticated() {
    given().when().delete("/raw-materials/{id}", 1L).then().statusCode(401);
  }

  @Test
  @TestSecurity(user = "user", roles = "USER")
  void delete_shouldReturn403_whenUserIsNotAdmin() {
    given().when().delete("/raw-materials/1").then().statusCode(403);
  }
  ...
  void findById_shouldReturn401_whenUserIsNotAuthenticated() {
    given().when().get("/raw-materials/{id}", 1L).then().statusCode(401);
  }
        ...
        .findAll(
            argThat(
                dto ->
                    dto.page == 1
                        && dto.size == 5
                        && "code".equals(dto.sortBy)
                        && "desc".equals(dto.direction)));
  }

  @Test
  void findByName_shouldReturn401_whenUserIsNotAuthenticated() {
    given().queryParam("name", "steel").when().get("/raw-materials/search").then().statusCode(401);
  }
